...
def train_func(env, current_model,target_model, optimizer, replay_buffer, device=device):
    steps_done = 0
    episode_rewards = []
    losses = []
    current_model.train()
...
def test_func(env, model, episodes, render=True, device=device, context=""):
    env = gym.wrappers.Monitor(env, VIDEO_SAVE_PATH + f'ddqn_{env.spec.id}_video_{context}', force=True)
    model.eval()
    for episode in range(episodes):
        state = env.reset()
        episode_reward = 0.0
